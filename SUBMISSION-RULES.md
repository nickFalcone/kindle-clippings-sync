...
| --- | --- | --- |
| PG-9 — `obsidianmd/settings-tab/prefer-setting-definitions` and `@typescript-eslint/no-deprecated` | Carry two lint warnings | One cause: the tab renders through `display()`, which Obsidian deprecated in 1.13.0 in favour of `getSettingDefinitions()`. Its settings therefore don't appear in settings search on 1.13+. Full migration means dropping `display()` and raising `minAppVersion` from 1.4.0 to 1.13.0, abandoning users below it; implementing `getSettingDefinitions()` alongside `display()` keeps 1.4.0 (nothing calls the new method before 1.13) and would restore search while leaving only the deprecation warning, at the cost of describing every setting twice. `prefer-setting-definitions` is on the plugin's `no-restricted-disable` list, so `eslint-disable` is itself an error, and a stub returning `[]` only hides it — it checks that the member exists, never the return value. `npm run lint` reports exactly these two warnings and exits 0. The scorecard surfaces lint warnings publicly, so both will be visible on the plugin's directory page. Revisit when `minAppVersion` moves to 1.13+, or if settings search outweighs the duplication. |
...
| 2026-08-01 | `5ece34a` | PG-9 rationale corrected after reading the rule source and the 1.13.1 typings rather than restating the earlier assumption. "Adopting the declarative API requires `minAppVersion` 1.13.0" is true only of a full migration: `getSettingDefinitions()` is called by `addSettingTab()` and the base `display()`, neither of which exists before 1.13, so an additive implementation would be inert on older versions and need no manifest change. Also found that the `obsidian` devDependency resolves to 1.12.3 (1.13.1 is published) and has no `SettingDefinitionItem`, so neither path is available without a bump, and that `SettingDefinitionRender` exists — the Browse and Sync now buttons would not block a migration. Warning still carried; the reasons in `src/settings.ts` and `CONTRIBUTING.md` now match the evidence. |
| 2026-08-01 | `5ece34a` | `obsidian` devDependency pinned from `"latest"` to `^1.13.1`. It was resolving to 1.13.1 on a fresh `npm install` while the lockfile held CI at 1.12.3, so contributors and CI type-checked against different API surfaces. Verified before pinning: `tsc` clean, 74 tests pass, and `obsidianmd/no-unsupported-api` — which builds its `@since` map from the installed typings and compares against manifest `minAppVersion` — reports nothing, so no API in use is newer than 1.4.0. The bump does surface a second standing warning: 1.13.0 marks `PluginSettingTab.display()` `@deprecated`, which `@typescript-eslint/no-deprecated` flags on our override. Accepted and documented rather than hidden by staying on old typings, which would not have hidden it from the scorecard. PG-9 updated to cover both warnings. |
